...
```bash
python ~/Code/Split_RNA_dataset/Version_2/split_rna_dataset_v1.py --data-dir ~/Data_V2 --report-root ~/Code/pipeline_reports --rank1-annotation-only --threads 16
```
...
```bash
python ~/Code_Flow_matching/scripts/build_refinement_pt.py --prediction-root ~/Data_V2 --native-root ~/pdb_data --rnafm-root ~/Data_FM/RNA_FM_embeddings --output-root ~/Data_PT_V2 --high-rmsd-root ~/Data_PT_V2_RMSD_GT30 --log-dir ~/Code/pipeline_reports/PT_V2 --run-name write_v2_rmsd30_20260924 --ccd-components-file ~/protenix_data/common/components.cif --max-pre-refinement-rmsd 30
```

默认的 `refinement_excluded_pdb_ids_v1.tsv` 仍按 PDB ID 排除其列出的数据质量问题；日志会记录原策略中的 split 和本次实际 split。主训练 PT 根目录不会包含 >30 Å 样本。检查 `summary.json` 中 `issues`、`failed_samples`、`rmsd_filtered_samples` 和 `high_rmsd_saved_samples`，后两项应一致。重跑前应使用新的 run name；只有明确需要重建样本时才加 `--overwrite`。
