...
1.测其他能够整个cif输入 cif输出的
    -->V1版本可以测单链输入单链输出的了
2.与Alphafold的扩散模型竞争

3.可以把protenix跑出来的decoys先一段短的MD  然后抽20帧(应该不用了，因为protenix即使是对包含在测试集中的数据，RNAfolding的效果也不是很好)

 完成对齐 生成.pt文件(确定元素) 
 看模型 之前之后的架构 RNA-FM已加 | extra知识 再想创新点 改一改模型(主要改进模型)
