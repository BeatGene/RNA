...
## 2026.3.17
1.完成网页端的结构预测 今天的量已经完成了   共177个 也已经上传到服务器 `DONE`

2.把目前下载下来的九千多个cif文件变成简单的protenix输入Json文件  正在跑(是在旧的实例上跑)  `DONE`

3.看能不能通过macbook的vscode连接服务器   不能 但是通过pycharm成功连接了 `DONE`

4.看能不能在一个新的实例上安装protenix 注意1.更新pypi 2.安装1.0.0的protenix  安装了 正在尝试跑 (是在新的实例上跑) 成功了 之后大量跑 `DONE`
...
1.完成网页端的结构预测  已完成并上传  175+100=275  `DONE`

2.用脚本在新的实例上跑 先跑prep 再跑pred 从后往前跑 从9开头的文件开始   正在新的实例上跑  跑完了 花了25小时 `DONE`
...
1.多开几个实例跑预测  9已经跑完 1在网页上跑的 8 7 6 5正在跑`DONE`

2.完成网页端的结构预测  已完成并上传  以1开头的全部是用网页端预测的 `DONE`
...
3.用降级版本的跑:  没必要了，成功在新的实例上安装了protenix的1.0.0版本 ✔
要求是CUDA12
...
1.看了RESM和RNA-FM 二者都是对残基/核酸苷进行编码。后续先继续理清目前RNA的整体架构，看看能不能缝进去  还差Data文件夹没有上传到新服务器
...
1.把Data文件夹下载到本机并且上传到新服务器

2.做Bio的答辩PPT，把Bio项目docker化。



### 2026.7.20 & 21

1.看RNA项目的代码 比较改了之后的模型和原模型的整体架构  看看能不能把大模型提取特征加进去

2.搬运Data

3.安装Codex  GPT-5.5   5.6
...
4.看foldbench是怎么设定的（protenix的预测参数的设置） 包括生成decoys的时候和看模型预测能力的时候

5.不要4*50   50*4  种子多

6.先把这么多cif每个正常预测（一个种子 五个sample） 然后选最好的跟GT比较 看看现在的模型的RMSD效果怎么样  （包括测试集

7.可以把protenix跑出来的decoys先一段短的MD  然后抽20帧？

======================================================================================================================================


1.看RNA项目的代码 比较改了之后的模型和原模型的整体架构   两部分 模型架构 丨 数据   数据稳步制作中ing  prep RNA-FM
...
1.继续RNA refinement项目   RNArefine不足为惧  测试集的选择可以借鉴一下(实际不行)

[//]: # (2.熟悉af3相关的代码和文献，把原版，protenix，boltz2的文章都看一看)

### 二类：
1.AI Agent: 已问项目(算法) 从小红书 知识星球 买一个项目(根据自己的目标方向) 

2.Java学习路线 **后端**全家桶(**开发**)  抓紧
